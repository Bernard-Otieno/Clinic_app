...
- **Choosing UUIDs over sequential IDs** for every table's primary key. Sequential IDs are guessable and can leak information — if a patient's appointment is `/appointments/47`, someone could simply try `/appointments/48`, `/appointments/49`, and so on. This came from experience with similar systems I've built before, where UUIDs help prevent people from accessing information they shouldn't be able to.

One side effect of that second decision: UUIDs are long and hard to visually tell apart, so I built the lightweight frontend specifically to help me confirm my own backend logic was working correctly — I couldn't easily eyeball which doctor, patient, or slot I was looking at from raw UUID strings alone in `/docs`. It ended up being useful for demos too, but testing my own booking logic was the actual reason I built it.