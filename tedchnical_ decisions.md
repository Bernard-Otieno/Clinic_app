...
### 16. Lightweight calendar frontend added for my own testing (not a graded requirement)
...
**Reasoning:** This is a Backend Developer assessment; the frontend isn't graded. I built it because I was using UUIDs as primary keys, and UUIDs are long and hard to visually tell apart — I couldn't easily "master who is who" just by reading raw IDs in `/docs` or Supabase's Table Editor. A visual calendar let me actually see which doctor/patient/slot I was working with and confirm the booking logic was behaving correctly, rather than cross-referencing UUID strings by eye. It turned out useful for demoing too, but that wasn't the original reason I built it. Keeping it same-origin (served by FastAPI itself) avoided CORS complexity and a second deployment target.
